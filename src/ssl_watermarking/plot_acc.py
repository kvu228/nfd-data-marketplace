...
output_dir = './output'  # Script runs from wm_codes directory
...
for q in quality_list:
    df_filename =  f'df_agg_{q}_6.csv'  # Updated to match output-a file format
    df_name = os.path.join(output_dir,df_filename)
    # Read CSV with multi-index header (rows 0 and 1 are headers, row 2 is column names)
    df = pd.read_csv(df_name, header=[0,1])
    df_list.append(df.values)
    # Access ID_acc mean column using multi-index
    try:
        id_acc_mean = df[('ID_acc', 'mean')]
    except KeyError:
        # Fallback: if multi-index not working, try single level
        id_acc_cols = [col for col in df.columns if 'ID_acc' in str(col)]
        id_acc_mean = df[id_acc_cols[0]] if id_acc_cols else df.iloc[:, 10]
    acc_list.append((pd.to_numeric(id_acc_mean).mean(axis=0))*100.0)
    # Access pred_acc mean column
    try:
        pred_acc_mean = df[('pred_acc', 'mean')]
    except KeyError:
        pred_acc_cols = [col for col in df.columns if 'pred_acc' in str(col)]
        pred_acc_mean = df[pred_acc_cols[0]] if pred_acc_cols else df.iloc[:, 14]
    value_attacks = pd.to_numeric(pred_acc_mean)
